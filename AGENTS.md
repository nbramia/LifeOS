...
| `mcp_server.py` | MCP server — stdio for Claude Code + HTTP transport for Managed Agents (77 tools: 68 from `CURATED_ENDPOINTS` plus 9 `lifeos_agent_*` tools registered separately by `_register_inter_agent_tools()` from `INTER_AGENT_TOOL_SCHEMAS`) |
