...
- **Co-scheduling a `browser`-marked file with an unrelated `async def` test can hide a bug behind file order.** `--dist loadscope` groups tests by test *class*, not by file, and hands scope groups out to workers **in collection order** — the order files are given on the command line. A file listed first has all its classes handed out before a later file's, so a bug that only manifests once a browser test's fixtures have *already run* in a worker process (state or resource leakage into whatever the worker executes next) goes unnoticed whenever the reproducer lists the affected file first — no browser class has had a chance to run before it. When bisecting a co-scheduling flake, list the browser files **before** the suspect file, not just alongside it, e.g.:
  `pytest tests/some_x_ui_browser.py tests/the_suspect_test.py -n 4` (browser file first), in addition to the reverse order. The concrete hazard this guards against: pytest-playwright's sync-API driver connection parks an event loop for its fixture's whole lifetime, which leaves asyncio's per-thread "running loop" marker set and makes any `async def` test that follows in the same worker fail with `Runner.run() cannot be called from a running event loop`. `tests/conftest.py` re-scopes those fixtures from session to module so the loop is torn down at the end of each browser file; the comment there carries the mechanism and the exact reproducer. Neither `scripts/test.sh`'s separate unit/browser invocations nor a suspect-file-first reproducer exercises this path.
