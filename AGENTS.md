...
- **Co-scheduling a `browser`-marked file with an unrelated async test can hide a real bug behind file order (#916).** `--dist loadscope` groups tests by test *class*, not by file, and hands scope groups out to workers **in collection order** — the order files were given on the command line. A file listed first has all its classes handed out before a later file's, so a bug that only manifests when a browser test's fixtures have *already run* in a worker process (state or resource leakage into whatever the worker executes next) can go completely unnoticed if the reproducer happens to list the affected file first — the browser class simply never gets a chance to run before it. When bisecting a co-scheduling flake, try the browser files listed **before** the suspect file, not just alongside it, e.g.:
  `pytest tests/some_x_ui_browser.py tests/the_suspect_test.py -n 4` (browser file first) in addition to the reverse order. #916 itself was a Playwright sync-API driver connection (session-scoped upstream) leaving asyncio's per-thread "running loop" marker set for the rest of a worker's life, which broke an unrelated `async def` test that happened to run afterward in the same process — invisible under `scripts/test.sh`'s normal, separate unit/browser invocations, and invisible under the co-scheduled reproducer too until the browser files were listed first.
