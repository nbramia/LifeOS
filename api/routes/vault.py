...
this endpoint, *when the journal persona is enabled on this install*: those
files are generated by `gsheet_sync.py` from a Google Sheet and
...
surfaces that produce it is actually configured on this install: the
...
def _journal_enabled() -> bool:
    ...
    resolving personas more indirectly — this is intentional, not a gap: a
    bot bound to the journal persona under a different registry name would
    not match, because `"journal"` is a fixed, hardcoded identifier this
    exact string is compared against everywhere the journal persona is
    used — `journal_capture.JOURNAL_PERSONA_ID`,
