...
The bottom of the Board tab keeps a touch-sized assignee tray with one control per supported assignee. Dragging a control onto an eligible task card applies the same assignment write and policy checks as the drawer; selecting a control and then activating a card is the accessible click/tap alternative. Refused targets expose the server-provided reason and do not write. When the Done lane is hidden, the tray also keeps a compact Done target available for permitted lane moves; dropping a Review card there uses the same acceptance transition as **Accept**.
...
A blocked card's existing **Answer** action is a note composer; submitting it deposits the note into the open question and lets the worker resume the prior session. A Review card with a linked prior session also offers **Reject** (required note, resumes In progress) and **Reassign** (valid assignee plus optional context note, moves to Assigned while preserving prior-run transcript and messages). These actions are shared by the Board drawer and Graph session panel and report server-side failures without moving the card. Human queue cards label their manual completion action **Mark Done**.
