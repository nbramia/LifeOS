...
A card shows its title, assignee chip, model/effort chips when the task carries those fields, a host chip, its other tags, and a pulsing dot when a linked session is actively running. The host chip carries two distinct, separately-rendered meanings: an assigned-host chip when the card's host field names a machine other than the one running the API — the assignment, where the card will run — and a ran-on chip when a linked session actually ran somewhere — the observation, where it did run. A card carries both, visually distinguishable, when the two differ; it carries neither when it has no host field and no linked session has run.
