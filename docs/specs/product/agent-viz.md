...
- **Routing** — a plain badge, one of `Local`, `Claude Code`, `Codex`, `Remote`, `Hermes`, `Ask` (parked waiting on the operator, no model running), or `Claude` — never a model name; this badge shows the raw `routing` value only, always plain, for every routing including `Hermes`. The graph node's `model_label` badge is the richer one for most routings — `Remote` becomes the configured remote provider's own label, and `Hermes` becomes `Hermes · <model>` once the session has taken a turn — see **Node label** in [Graph tab — what you see](#graph-tab--what-you-see). The model shown there comes from `Session.hermes_model`, recorded per session rather than read from any process-wide "last observed" value, so a finished session's badge cannot show a model that an unrelated Hermes turn reported; a session that hasn't taken a turn yet still shows plain `Hermes`.
