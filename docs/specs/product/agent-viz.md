...
- **Routing** — a plain badge, one of `Local`, `Claude Code`, `Codex`, `Remote`, `Hermes`, `Ask` (parked waiting on the operator, no model running), or `Claude` — never a model name; this badge shows the raw `routing` value only, always plain, for every routing including `Hermes`, computed client-side and never reading `model_label`. `Session.hermes_model` records the model per session (written only by that session's own turn), and the snapshot's `model_label` field carries `Hermes · <model>` once a session has taken a turn — but this is a data-layer detail only, not a visible one: `model_label` is consumed in the UI solely as the graph node's fifth-priority NAME fallback (behind an operator-pinned custom label, the AI-generated short summary, the derived label, and the prompt preview — see **Node label** in [Graph tab — what you see](#graph-tab--what-you-see)). A board-assigned Hermes card always has a derived label (its card title), so it takes that branch first and never reaches `model_label` — the enriched value exists in the API response but does not currently render anywhere on screen.
