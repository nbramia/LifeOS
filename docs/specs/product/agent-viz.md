...
| **Snoozed** | The task carries a future wake-up time and wins over every other lane rule regardless of status or tags; the card returns to its natural lane on its own once that time passes, with no other change to the task. Any Unassigned, Assigned, Human queue, or Review card can be snoozed; In progress, Done, and scheduler entries cannot. Set and cleared through `PUT`/`DELETE /api/agents/board/cards/{id}/snooze`; the board has no lane-visibility toggle or drawer control for it. |
