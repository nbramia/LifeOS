...
Each card shows the entry's next fire time, a recurring badge for cron entries, and — once it has fired at least once — the most recent run's outcome and a short result snippet. The drawer edits the whole schedule, saving through the same `PUT /api/scheduler/{id}` the `/api/scheduler` UI uses — there's no separate write path for the board: name, message, and an enabled checkbox; schedule type (cron or one-off); the schedule value (a cron expression or an ISO datetime — the field's label and placeholder switch with the type); an IANA timezone; the action (`notify`, `prompt`, `endpoint`, or `agent`); an executor picker shown only when the action is `agent`; and a bot picker shown only otherwise, listing exactly the names `GET /api/scheduler/bots` returns plus an empty option meaning the primary bot, so picking a name the API would reject is impossible. Switching the action select shows and hides the executor and bot controls immediately, without reopening the drawer. Each field saves on blur (text) or change (selects, the checkbox); a save the server rejects — an unparsable cron expression or datetime, an unknown timezone — shows the response's detail inline next to that field and reverts it to the last value the server accepted, the same disable-and-explain convention the task drawer's pickers use for policy reasons. A human-readable next-fire preview updates from the response of a schedule-type, schedule-value, or timezone save. A **Trigger now** button calls the test-fire endpoint (`POST /api/scheduler/{id}/trigger`) and refreshes the drawer's last-run line; a failed trigger shows a toast with the reason. The drawer's Delete action (see [Cards and the drawer](#cards-and-the-drawer)) is offered here too, behind the same confirmation, and removes the entry through `DELETE /api/scheduler/{id}`.
