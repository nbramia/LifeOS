...
### P10.4: Dunbar Circle System

**Requirements:**
Contacts are assigned to Dunbar circles (0-7) based on relationship strength ranking. Circles are used for filtering, color coding, and prioritization throughout the CRM.

**Circle Assignment:**
- Circles 0-6 represent progressively outer rings of social connection (0 = closest)
- Circle 7 = peripheral contacts (below `PERIPHERAL_THRESHOLD`)
- Calculated in `relationship_metrics.py` based on relationship strength ranking among non-peripheral contacts
- Configurable overrides via `CIRCLE_OVERRIDES_BY_ID` in `config/relationship_weights.py`

**UI Integration:**
- Dunbar badge on people cards in list view (color-coded per circle)
- Dunbar circle indicator in person detail header
- Circle filter dropdown in graph view to filter nodes by circle
- Graph color mode toggle: category colors vs Dunbar circle colors

### P10.5: Graph Visualization
...
**Graph Enhancements:**
- **Fullscreen mode**: Toggle button expands graph to fill viewport (mobile and desktop)
- **Color modes**: Toggle between category-based colors (work/personal/family) and Dunbar circle colors
- **Node sizing**: Toggle between strength-based sizing and centrality-based sizing
- **Edge detail panel**: Click an edge to see relationship breakdown (shared events, threads, messages) with copy and navigation actions
- **Dunbar circle filter**: Multi-select dropdown to show/hide nodes by circle
- **Fit-to-first-degree**: Graph layout prioritizes first-degree connections of the selected person
...
[ ] Fullscreen toggle expands graph to fill viewport
[ ] Color mode toggle switches between category and Dunbar colors
[ ] Node sizing toggle switches between strength and centrality
[ ] Clicking edge opens edge detail panel with source breakdown
[ ] Dunbar circle filter shows/hides nodes by circle
[ ] First-degree edges rendered distinctively (on top, thicker)
[ ] Performance: renders <2s for 30 nodes
```

**Test File:** `tests/test_crm_ui_graph.py`

**Completion Promise:** `<promise>P10.5-GRAPH-UI-COMPLETE</promise>`
...
- Floating toolbar appears when 1+ people are selected, showing count and action buttons
- Actions: Merge selected (combines into one PersonEntity), Hide selected (soft-deletes)
- Clear selection button to deselect all

### P13.5: Cleanup Modal

**Requirements:**
Review queue for data quality issues: duplicates, non-human entities, and over-merged records.

**Tabs:**
- **Duplicates**: People that may be the same person (fuzzy name match, shared identifiers). Actions: merge or dismiss.
- **Non-Human**: Entities detected as non-person records (mailing lists, bots, system addresses). Actions: hide or dismiss.
- **Over-Merged**: People with suspiciously many identifiers that may be incorrectly merged. Actions: split or dismiss.

Each tab shows a badge with the count of pending items. Items are sourced from `api/services/review_queue.py` via `GET /api/crm/cleanup/pending`.

### Implementation Files

| File | Purpose |
|------|---------|
| `api/routes/crm.py` | Contact sources endpoint, split endpoint, cleanup endpoints |
...
| `api/services/review_queue.py` | Cleanup review queue (duplicates, non-human, over-merged) |
| `web/crm.html` | Split modal, merge toolbar, cleanup modal UI |
...
## Phase 16: Me Dashboard

### Overview

The Me Dashboard (`/me` route) is the default landing page for the CRM. It shows the owner's network health at a glance, replacing the generic people list as the entry point.

**URL:** `/me` (redirected from `/crm` automatically)

### P16.1: Me Dashboard View
...
- **Messaging by circle**: Interaction volume broken down by Dunbar circle
...
- `GET /api/crm/me/interactions` — Interaction data with trend and health period support (used by heatmap, volume chart, trend visualizations)

### Implementation Files

| File | Purpose |
|------|---------|
| `api/routes/crm.py` | `/me/stats`, `/me/timeline`, `/me/interactions` endpoints |
| `web/crm.html` | Me dashboard UI, `renderMeDashboard()` |

---

## Phase 17: Birthdays Page

### Overview

Dedicated birthdays view showing a heatmap of contact birthdays across the year and a timeline of upcoming birthdays.

**URL:** `/birthdays`

### P17.1: Birthdays View
...
**API Endpoints:**
- `GET /api/crm/birthdays/today` — Contacts with birthdays matching today's date (used for toast notifications)
...
| File | Purpose |
|------|---------|
| `api/routes/crm.py` | `/birthdays/today`, `/birthdays/all` endpoints |
| `web/crm.html` | Birthdays page UI, heatmap, timeline |

---

## Phase 18: Relationship Dashboard

### Overview

The Relationship Dashboard (`/relationship` route) provides therapy-informed insights and communication visualizations for a primary relationship (e.g., partner).

**URL:** `/relationship`

### P18.1: Therapy Insights
...
### P18.2: Communication Visualizations
...
| File | Purpose |
|------|---------|
| `api/routes/crm.py` | `/relationship/tone-analysis`, `/relationship/tone-analysis-detailed` endpoints |
| `api/services/relationship_insights.py` | Therapy insight extraction |
| `web/crm.html` | Relationship dashboard UI, visualization rendering |

---

## Related Documents

- [API Reference](api-reference.md) -- CRM API endpoint contracts
