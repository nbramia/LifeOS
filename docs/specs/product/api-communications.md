...
`cost_usd` on the `usage` event is omitted (not `0`) for a turn this server can't price — e.g. the `remote` model pick with no configured rate — so a client must distinguish "absent/non-numeric" (unpriced) from a real `0` (genuinely free) rather than treating both as free; `web/chat/ask-stream.js` already does this.
...
- `conversation_id` (optional, default none) — scopes `session_cost_usd` and friends to that conversation's already-recorded usage. Omitted, or a conversation with no recorded usage yet, reports those fields present and zero rather than omitting them.
...
`/chat` reaches three transport surfaces through LifeOS reverse proxies so the browser stays same-origin. The shapes are owned upstream; LifeOS only forwards.
