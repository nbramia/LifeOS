...
| `POST /api/crm/relationship/tone-analysis-detailed` | Separate user/partner tone scores, one overall score per person per stale month. Persisted per person/month in `crm.db`'s `tone_analysis_results` table (one row per person + `YYYY-MM` period); a stored month is reused as-is when its interaction count hasn't changed and it was computed within the last 30 days, so a page load with nothing new to analyze never calls the LLM. Every stale month in a request is scored in a single batched LLM call (not one call per month), and concurrent requests for the same person are serialized so two open tabs (or a double-clicked refresh) don't pay for the same call twice. `refresh=true` forces every month in the window to recompute. A month that can't be computed (the LLM call failed, or its response omitted that month) comes back marked `status: "error"` rather than failing the request or fabricating a score — the Tone Evolution chart shows these as a gap with a "not analysed" marker rather than plotting them. Trend labels (`user_trend`, `partner_trend`, `combined_trend`) are computed locally from the stored monthly scores (comparing the first half of the window's average to the second half) rather than authored by the LLM, since a response can be assembled from a mix of stored and freshly recomputed months. |
