...
**Preset class (Jev).** `_apply_preset_class` sets `result.preset_class` from, in order: an explicit `#<class>` tag (always wins); else a Jev fan-out judgment (`jev_task_routing.judge_task`) when `TYPESAFE_API_KEY` is set — its `preset_class` choice when confidence is >= 0.6, else explicitly `"fullstack"`; else left unset, same as an install with no key. The judgment is one call shared with working-directory and plan-mode resolution (see "Working directory" below and [agent-worker-setup.md](../../guides/agent-worker-setup.md#typed-judgments-jev)) — `judge_task` is memoized per task title so the three consumers don't triple the call count.
...
`resolve_working_directory(task)` asks the Jev fan-out judgment (`jev_task_routing.judge_task`) first: a `location` answer with confidence >= 0.6 resolves against the union of the operator's GitHub repositories (`directory_resolver._github_repos()`, `gh repo list <owner>`, disk-cached 24h), the scanned local project directories, LifeOS, the vault, and home — a repo Jev names that isn't cloned on this host still resolves to a path under `code_dir`. Below that confidence, or with no Jev judgment at all, it falls back unchanged to the keyword cascade. For the CLI routes specifically, the worker clones an uncloned repo (`directory_resolver.ensure_cloned`, `gh repo clone <owner>/<name>`) right before spawning, but only when the executing host is this process itself — a session dispatched over `LIFEOS_AGENT_HOSTS` to a remote host never attempts a clone here, since the CLI's filesystem is on the other machine; a clone failure parks the task at `#agent-blocked` naming the repository instead of spawning into a missing directory.
