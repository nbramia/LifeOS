...
The `tailscale` subprocess itself degrades to `[]` peers on every expected failure mode (missing binary, non-zero exit, unparseable JSON, timeout) rather than raising — including non-UTF-8 output on stdout/stderr, which `_default_status_runner` no longer asks `subprocess.run` to decode (`text=True` decodes eagerly and raises `UnicodeDecodeError` — a `ValueError` — before `_probe_peers`'s own except tuple gets a chance); `_probe_peers` keeps raw bytes and decodes with `errors="replace"` itself, and that except tuple also catches `ValueError` as a belt-and-braces (#901 round 2, finding A2).

A `_build()` that raises anyway (not a probe failure — something else going wrong) is handled INSIDE `get()`'s lock rather than propagating to every waiter: round 1's coalescing lock only helped the success path, so a raising build re-ran once per queued waiter (measured: 30 concurrent failures went from 1.2s/30 probes with no lock to 36s/30 probes with the naive lock). The fix caches a probe-free `degraded()` snapshot — every registry host still listed, each at `online: null`, reusing `_build`'s own host-assembly logic with an empty peer list rather than a separate code path — under a much shorter `_HOST_CATALOG_NEGATIVE_TTL_SECONDS` (5s), so a failing probe costs one attempt per negative-TTL window instead of one per waiter (#901 round 2, finding A4). The route (`agent_assignment.py`) wraps `catalog.get()` in `asyncio.wait_for(..., timeout=_HOSTS_ROUTE_TIMEOUT_SECONDS)` (1.8s) as a real ceiling — the old "2-second budget" was only ever inferred from the tailscale probe's own 1.5s timeout, which doesn't cover lock queueing or event-loop scheduling (measured up to 2.57s under concurrent load) — and falls back to the same `degraded()` snapshot on a timeout or any other exception, logged at `error` with a traceback rather than a quiet `warning` (finding R5; the pre-round-2 backstop returned just the API host, dropping the registry entirely).
...
**Client-side caching** (`web/agents/assignment.js`'s `loadHostCatalog`) is a *short-TTL* cache (`_HOSTS_CLIENT_TTL_MS`, 30s — matched to the server's own `_HOST_CATALOG_TTL_SECONDS`), not the once-per-page-load cache the model catalog uses: reachability markers go stale in seconds, so a drawer opened well after the client TTL re-fetches rather than showing page-load-time data for the rest of a long-lived session. A fetch failure is never cached — the caller for that one call gets a `null` catalog back (the select falls back to its synchronous seed: "this machine" plus the card's saved host, if any, flagged unknown, now joined by a disabled "hosts unavailable — reopen to retry" option so a failed load doesn't read as "nothing is registered" — #901 round 2, finding R7), but the next drawer open retries rather than being stuck with an empty picker for the rest of the session. After a SECOND consecutive failure, a 10-second cooldown (`_HOSTS_FAILURE_COOLDOWN_MS`) suppresses further fetches so a permanently-dead endpoint costs one request per cooldown window rather than one per drawer open; a single transient blip still retries on the very next drawer open (finding R6). A slow fetch that fails after a newer, faster fetch already cached a success no longer clobbers that newer cache entry — the `.catch` only clears `_hostsCache` if it's still the entry that fetch itself installed (finding M6).

A host change the server rejects — and, alongside it, whatever effort/model change rode in the same save — reverts the select(s) to their last-known-good value before the error is surfaced, the same way board.js's other drawer controls already revert on a failed save; without this, a rejected host silently rode along on the next unrelated save with no toast at all (#901 round 2, finding A3).
