...
effort and host fields. Its explicit working directory wins; otherwise a
recognized project affinity supplies a location only for an executor that
supports `working_dir` on the API host. Managed Agents ownership uses the
explicit consent alias's model and retains configured effort and host, but not
the configured model, working directory, or affinity fallback, as described in the
