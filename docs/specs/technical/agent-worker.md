...
- **The API needs consent (#584).** `routing=claude` is the only per-token-billed route, so preflight may reach it only when the operator asked: a `#cloud` / `#cloud-haiku` / `#cloud-sonnet` tag, or a title naming an engine or model (the classifier's `routing_explicit`, corroborated against the title so a hallucinated flag can't dispatch). A cloud route the classifier *inferred* — rule 4's capability cues — is downgraded to `ROUTE_ASK` and confirmed. The confirmation offers `claude code` / `codex` / `local` / `cloud`, and a bare "claude" in the reply resolves to the **CLI**, not the API: with both in play, the subscription reading is the one where a misparse costs nothing.
