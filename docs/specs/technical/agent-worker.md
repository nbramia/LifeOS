...
still has commits the default branch doesn't, `"merged"` once it doesn't —
computed via `git_worktree.repo_default_branch`/`repo_compare_ahead_by`
against a repository slug derived from any project child's recorded PR URL,
cached per project for `_INTEGRATION_AHEAD_CACHE_SECONDS` (the same window
`_cleanup_session_resources` caches a child's own PR state for) since this
check runs every tick for every in-flight agent-owned project with a
recorded branch. No coding child PR on record yet, or the remote check
itself failing, contributes no event that tick — never a false `"merged"`.
Because it reuses the exact same "active" baseline / non-"active" event
