...
- **Doctor (`orchestrates: true`).** A fresh message to the doctor bot now flows through the same chat pipeline as every other persona bot — the direct-Claude-Code spawn entry that used to bypass chat entirely (`_handle_orchestration_message`) is retired. On Hermes, the doctor persona supervises its own workers via `lifeos_agent_spawn` (`config/personas/doctor.hermes.md`, hermes#62/#642) inside its own ordinary streamed reply — no client-side spawn logic left in `telegram.py` at all. On a native fallback turn, sending `persona_id="doctor"` (not raw preamble text) is what makes `POST /api/ask/stream`'s own persona_id-gated orchestrating-persona spawn (`settings.persona_orchestrates()`, in `api/routes/chat.py`) fire — the same mechanism the web/voice surfaces already use for this persona (see the Persona contract, above) — tagging the spawned session `bot="doctor"` so Telegram-thread parity holds. **The threaded-reply resume hooks and status-anchor machinery are unchanged** — `_owns_agent_sessions` (primary + any `orchestrates: true` bot) still gates them, and they resume a `lifeos_agent_*` child session regardless of who spawned it (Hermes, or the native fallback's spawn). Only the *fresh-message* entry point changed. **`orchestrates`'s meaning elsewhere is untouched**: `settings.persona_orchestrates()`, `GET /api/personas`, and the web UI's own orchestration path (documented in the Persona contract above) are unaffected — this section changes only what the Telegram *listener* does with a fresh message to an `orchestrates: true` bot, not the flag's meaning anywhere else.
- **`claude_intent` and the "code task → main bot" redirect.** The Hermes backend never emits a `claude_intent` event (Hermes has no engine-handoff concept of its own), so `chat_via_api()`'s SSE parsing loop simply never sets it — the listener's existing `result.get("claude_intent")` check already tolerates its absence with no special-casing. The accepted, deliberate side effect: a specialized bot's "that looks like a coding/agent task — send it to your main LifeOS bot" redirect only ever fires on a native-pipeline turn (the primary bot, or a specialized bot's fallback turn) — a hermes-backed specialized bot no longer redirects an inferred code task, since Hermes never tells LifeOS it saw one.
- **journal.** The journal bot's turn — Hermes or native fallback alike — still captures the fragment before the model ever answers, and still requires the `journal_capture` proof event exactly as `chat_via_api()` and the ring ingest (`api/routes/journal_ingest.py`) always have; #685 is what makes the Hermes proxy emit that event at all. The ring ingest itself (a separate HTTP consumer, not a Telegram bot) is unaffected by this section — it has always called `chat_via_api()` directly with a raw persona preamble on the native pipeline, and still does.
...
- **Answering a spawned session's `[CLARIFY]`/`[GOAL]` from web/voice:** on a successful **LifeOS-backend** spawn the server links the conversation to the spawned agent session (`conversations.agent_session_id`). If that session emits `[CLARIFY]`/`[GOAL]`, the worker registers an open question keyed on the session (not just a Telegram `message_id`), and the web/voice client can answer it **without Telegram**:
  ...
  - This answer path is **backend-agnostic** in mechanism — it reads `conversations.agent_session_id`, never `backend` — but in practice only ever fires for a `lifeos`-backend conversation now that Hermes doesn't create a LifeOS-linked spawn of its own.
- **`backend` tag on `POST /api/ask/stream`.** An optional, generic field on the request model, used **solely** to label a newly created conversation for sidebar filtering — never to route, resolve a persona, or pick a model. Through #641 the `/chat` client set this itself (to `"hermes"`) when diverting an orchestrating persona's turn from Hermes, so the resulting LifeOS-native conversation stayed visible in the Hermes-filtered sidebar. #642 removed that divert and, with it, the client's only reason to ever send this field — `/chat` no longer sets it on any turn. The field itself stays on the request model (any HTTP caller may still tag a conversation this way) and is otherwise unchanged: omitted (still the only case the first-party client produces) tags the conversation `"lifeos"`. `GET /api/conversations` keeps its matching optional `backend` query filter (unset = unfiltered).
...
- `POST /api/voice/transcribe` — **bare STT, needed by the "Listening" wake-word dock toggle (#710) — not yet implemented in whisper-relay.** Multipart `audio` in, `{"transcript": "..."}` out; no LLM call, no TTS, no conversation/turn persistence (a wake check must never look like a turn to anything downstream, including #711's persistence tee, which keys off `done` events this route must never emit). `web/chat/voice.js`'s `checkForWakeWord()` calls it on every captured speech burst while Listening is on; a 404 today just means every wake check misses.

**Topology:** browser → LifeOS `/chat` → (proxy) → gateway → for the `lifeos` backend, the gateway calls back into LifeOS `/api/ask/stream` (the consumer-into-LifeOS direction above); for the `hermes` backend (#593), the gateway is *eventually* expected to call LifeOS's **own** Hermes proxy, `POST /api/hermes/ask/stream` — not the Hermes harness directly — setting `modality: "voice"` on that call so the persona's spoken-style rules are attached exactly like a native spoken turn and the same seam's persona resolution, `lifeos_context` envelope, and conversation persistence (see "The `lifeos_context` envelope" below) all apply, the same gap #590/#592 closed on the text path. **As of this writing that migration hasn't shipped** (`nbramia/whisper-relay#32`, still open) — the deployed gateway's Hermes adapter (`voice_gateway/adapters/hermes_backend.py`) calls the Hermes harness directly, silently, with none of the above (confirmed by reading that adapter while fixing #711, not the "must fail the turn visibly" fallback an earlier draft of this contract assumed). Conversation/persona listing is owned by LifeOS, not the gateway.

**Persistence tee (#711, [ADR-021](../../adr/021-voice-turn-persistence-tee.md)):** because a Hermes-backend voice turn doesn't yet reach `/api/hermes/ask/stream`'s own persister (above), `api/routes/voice.py`'s `_VoiceTurnPersister` tees `POST turn/stream`'s relayed SSE response directly, independent of whichever internal call path the gateway used. Trigger: the terminal `done` event's authoritative `data` — a turn that errors, is cancelled, or disconnects before `done` persists nothing (this is also what keeps a future bare-transcribe/wake-check call, #710, from creating a conversation). Guard: writes only when the turn's `backend` form field was exactly `"hermes"` — a `lifeos`-backend turn is already persisted by the native orchestrator (its `done` arrives only after the gateway's internal `/api/ask/stream` call, and that call's persistence, already completed), and an `agent`-backend turn is never persisted, matching that backend's section below. Writes go through the exact same `ConversationStore` calls `_HermesTurnPersister` uses, so grouping (one conversation per voice session) matches the text path. Extracting `backend` requires buffering `turn/stream`'s multipart body — the one path on this proxy that isn't a pure unbuffered stream; every other `/api/voice/*` path is unchanged. **Known residual gap:** once whisper-relay#32 ships, a hermes-backend voice turn would be observed by both `_HermesTurnPersister` (via the gateway's now-internal Hermes-proxy call) and this tee, double-appending that turn's messages — not addressed here; tracked for whisper-relay#32's landing.
...
One additive SSE event is emitted only on the `journal` persona's turns (#674): `journal_capture` (`path`, `created`), sent right after `conversation_id`, once LifeOS has written the fragment to `Personal/Log/YYYY-MM-DD.md` in code — capture no longer depends on the model calling a tool. It is proof of a completed write, not a status hint: `api/routes/journal_ingest.py` refuses to report `status: "logged"`, or to burn a delivery's idempotency key, without it. A capture failure is raised **before the stream opens**, so it surfaces as an HTTP 500, never as a mid-stream `error` frame. Clients that don't know this event type simply ignore it.
