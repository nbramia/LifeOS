...
**LifeOS endpoints the gateway calls** (shapes: [api-reference.md](../product/api-reference.md)): `POST /api/ask/stream`, `POST /api/chat/handoff`. At startup it may call `GET /api/personas` server-side for handoff capability caching (not exposed to browsers). `POST /api/chat/cancel` (`client_turn_id`) is the gateway's explicit stop path (`whisper-relay#37`) on barge-in and hangup (see "Turn lifetime and cancellation" below): a gateway not calling this endpoint on cancel loses correct barge-in outright, since a disconnect alone detaches the turn like any other modality, rather than stopping it.
...
- `GET /api/personas` lists selectable personas (`primary` + registry-defined specialized personas) without requiring Telegram credentials. The client renders these as a picker; ids are stable, labels are display-only. A missing bot token disables only that Telegram listener and outbound bot projection, not the persona on HTTP/voice/Hermes surfaces. The `primary` persona and orchestrating bots (e.g. the doctor self-repair bot) carry `handoff`/`agent` capabilities — gate any handoff UI on a persona's `capabilities`, not on a hardcoded `primary` check. Those UI affordances are not a tool inventory.
...
- **Orchestrating personas on Hermes vs. LifeOS.** An orchestrating persona selected on Hermes reaches the Hermes proxy exactly like any other persona — no diversion, no 400 — and Hermes conversationally triages, spawns, supervises, and reports on a worker itself, inside its own ordinary streamed reply, driving a background Claude Code worker (`lifeos_agent_spawn`) with a per-conversation `caller_session_id`. Doctor's Hermes-specific preamble (`config/personas/doctor.hermes.md`, resolved via `surface="hermes"`) describes that capability instead of the plain body's "you have shell access" framing. **The two backends offer genuinely different UX for the same persona, not a workaround and its real implementation:** LifeOS's spawn is fire-and-forget (an ack, then results via Telegram/`/agents`, with no mid-flight visibility or correction); Hermes drives the same underlying capability conversationally, with the operator able to steer it as it runs. LifeOS's own decision to keep this fire-and-forget path, deliberately, is documented in "LifeOS-backend orchestration is retained deliberately" below — it does not become dead code just because Hermes offers a better experience for it.
  ...
  - `lifeos_context.persona.orchestrates` in the Hermes envelope (below) can be `true` for an orchestrating persona — this is a **cross-repo contract detail**; see "The `lifeos_context` envelope" below for what it means and why it isn't a silent flip.
- **Answering a spawned session's `[CLARIFY]`/`[GOAL]` from web/voice:** on a successful **LifeOS-backend** spawn the server links the conversation to the spawned agent session (`conversations.agent_session_id`). If that session emits `[CLARIFY]`/`[GOAL]`, the worker registers an open question keyed on the session (not just a Telegram `message_id`), and the web/voice client can answer it **without Telegram**:
  ...
  - A `goal_approval` question gates one **numbered revision** of a durable repair record. Whichever surface answers it — a Telegram threaded reply or this endpoint — resolves to that exact revision and drives the same transition: approval is consumed once and replays the revision's stored resume action, a refinement retires it, and a duplicate or stale reply enqueues nothing and spawns nothing. Reply matching stays scoped to the asking bot and the addressed session, so a doctor gate and a primary-bot question never cross.
  - This is the **input** direction (answering / resuming). The complementary **output** direction (streaming the session's results back into the web thread) does not exist; results arrive via the bot's Telegram + the `/agents` page.

**Repair state on the agents snapshot.** Each session in `GET /api/agents/snapshot` carries a `repair` field. It is `null` for every session outside a doctor repair — which is every ordinary `#agent` task — and otherwise an object `{workflow_id, phase, waiting_reason, approved_version, evidence}`:

- `phase` is one of `diagnosis`, `awaiting_approval`, `implementing`, `reviewing`, `merging`, `deploying`, `verifying`, `shipped`, `failed`, `declined`, `cancelled`. Waiting is explicit: `waiting_reason` names what the repair is waiting on (`goal_approval`, or the first unmet completion requirement).
- `approved_version` is the goal revision currently authorizing work; `null` before approval.
- `evidence` is the structured result bundle collected for that revision — pull requests, the review outcome, the candidate verifier's lane result, the merged commit, the deployment verifier's `RuntimeEvidence`, and the revert handle. It is cleared when a new revision is approved, so a bundle only ever proves the revision it was gathered for. `shipped` requires the deployment evidence to agree on the merged commit, including the revisions the processes are actually running; a merge whose deployment failed reports the deployment failure instead.
- The field is additive: a client that ignores it is unaffected, and both the web and Telegram surfaces read the same repair record rather than deriving state from prose.
...
On the `lifeos` backend, an orchestrating persona keeps its spawn path rather than answering inline or being dropped from the picker, for two reasons:
...
- **The other two options actively regress something, for no offsetting gain.** Answering inline would mean either serving doctor's plain preamble (which claims shell/filesystem access this backend's inline orchestrator does not have — false and misleading) or authoring a third, LifeOS-inline-specific persona variant nobody asked for. Hiding the persona from the `lifeos` picker breaks the requirement that the picker's contents don't change based on the selected backend, and removes a working capability from any operator who prefers or is stuck on `lifeos`.
...
- **A client disconnecting does not stop the turn.** Closing the tab,
  backgrounding the app, or a network switch does not stop generation — the
  turn keeps running server-side and persists the complete reply, readable
  via `GET /api/conversations/{id}` once it's reopened. A connected client's
  frame sequence and bytes are unaffected — this is purely about what
  happens *after* a disconnect.
- **Voice behaves like every other modality.** whisper-relay's cancel
  gesture calls `POST /api/chat/cancel` with its `client_turn_id` (below)
  explicitly on barge-in and hangup, rather than relying on disconnect
  (whisper-relay#37). A voice turn's disconnect detaches and survives
  exactly like a text turn's; only an explicit cancel — the gateway's real
  barge-in gesture — stops it. See
  [ADR-020](../../adr/020-voice-cancel-gate-lifted.md) for the rationale.
...
- **`client_turn_id` and `POST /api/chat/cancel` (closes the
...
- **Cancelling a turn whose reader is still attached is a supported,
  explicitly-tested ordering** — not just "cancel after
...
- **No tag and no inheritable reply → the base case, by construction.** `persona_id` and `lifeos_context` are both `null`, and `text` is returned exactly as sent. Hermes needs nothing from this response in that case — no LifeOS-derived preamble, no envelope, no dependency on LifeOS being reachable.
- **An unrecognized `@tag` is a 400, not a silent no-persona.** `@nonsense do a thing` matches the tag grammar (starts with a letter, ends at whitespace) but doesn't match any configured persona id — this is rejected with `400 Unknown persona_id: 'nonsense'` rather than falling back to `persona_id: null`. Deliberately: a typo that silently became "no persona selected" would look like it worked, while the user's actual intent — routing to a specific persona — goes unhonored.
      ...
      "tool_capabilities": [
        {
          ...
          "actions": ["log", "update", "list", "history", "summary", "log_metric", "metrics", "get_profile", "set_profile", "readiness"]
        }
      ]
    },
    "turn": {
...
- `schema_version` is currently `1`. `caller_session_id` carries no version bump — it's purely additive, so a consumer validating `schema_version == 1` and ignoring unrecognized keys is unaffected.
...
- `persona.tool_capabilities` is an additive, stable list of persona operations derived from the intersection of the executable native handler catalog and the tools actually built by LifeOS's MCP/OpenAPI registration path. The static curated map is not sufficient: if an endpoint is absent from the current OpenAPI route set, the MCP builder omits that tool and the persona projection omits its actions too. It does not grant access, describe Telegram readiness, or replace progressive discovery. Native/MCP aliases live in `api/services/persona_capabilities.py`; fitness's `manage_workouts` maps to `lifeos_workout_manage`, including `set_profile`, when that live tool is registered. A tool or action missing from either live catalog is omitted, while a focused strict validator detects alias/action drift. An empty list makes no tool claim. Older consumers may ignore the field, and Hermes treats an absent field from an older LifeOS as no projected claims while retaining its normal `tool_search`/`tool_describe` path.
- **`orchestrates` reflects live persona state, coordinated with `nbramia/hermes` (commit `84ef8de`, deployed 2026-08-22).** `orchestrates` reflects `settings.persona_orchestrates(persona_id)` honestly and can be `true` for an orchestrating persona like `doctor`, sent deliberately because doctor genuinely does orchestrate and that is the entire point of routing it to Hermes. `true` means "this persona supervises workers through tools," and Hermes is expected to act on the Hermes-specific preamble (`persona.preamble`, above) rather than refuse the turn. **If Hermes's `lifeos_adapter/envelope.py` is ever rolled back past `84ef8de`** (which raises `OrchestratingPersonaError` on `true`), every doctor-on-Hermes turn fails — the two repos are coupled on this field and must move together.
- `turn` is a **sibling** of `persona`, resolved by `build_turn_context()` in `api/services/agent_system_prompt.py` — the same function [`GET /api/chat/turn-context`](../product/api-communications.md#get-apichatturn-context) returns, so every field *except* `caller_session_id` can never drift apart between the two. **Never merge `turn` into `persona`** — `persona` is stable across a whole conversation (cacheable), `turn` changes every turn (or, for `caller_session_id`, at least *could*); merging either into `persona` would invalidate a consumer's prompt cache on every turn. The `session_*` fields are exactly why a cumulative cost belongs here and not in `persona`; `caller_session_id` belongs here for the same layering reason even though its value is often stable across a conversation's turns (see below) — it's still a per-request concern, not a fact about the persona.
- `caller_session_id` is Hermes's identity for the `lifeos_agent_*` tool family (`lifeos_agent_spawn`/`check`/`send`/`kill`/`yield_until`/`transcript_read`/`sessions_list`/`user_ask` — see `api/services/agent_worker/inter_agent.py`), which `mcp_server.py` already advertises to Hermes over stdio MCP. Every one of those tools requires a `caller_session_id` argument that resolves to a real row in LifeOS's agent-worker `SessionStore`; without this field, every call fails with `no_caller`. Hermes should copy this value verbatim into that argument on every `lifeos_agent_*` call it makes during the turn. **Present only in the Hermes envelope** — `GET /api/chat/turn-context` has no session to hand out and does not return this field.

**Validation and rejection**, in order, before any upstream request is made: malformed JSON → 400; unknown `persona_id` → 400 naming the id. An orchestrating persona is not rejected here. Attachment size/type caps are enforced via the same `AskStreamRequest` model the native endpoint uses, so the limits can't drift between the two paths.
...
| `session_cost_is_lower_bound` | boolean | always | `true` when any turn summed into `session_cost_usd` is `unpriced` — the provider reported no cost for it, recorded as `cost_usd=0.0` alongside the flag rather than as a real zero. `false` means every *recorded* turn in the sum was either genuinely priced or genuinely free — but see the retroactive-gap note below; it does not mean the figure is guaranteed exact. |
| `caller_session_id` | string | always, Hermes envelope only | Hermes's `lifeos_agent_*` identity for this turn — see "Hermes agent-worker session identity" below. Not part of `build_turn_context()`; not returned by `GET /api/chat/turn-context`. |
...
#### Session-to-date cost

A Hermes session has no built-in way to answer "what has this cost?" even though the browser header already shows a running total (`web/chat/ask-stream.js`'s `state.sessionCost`, accumulated client-side from each turn's `usage` SSE event). The five `session_*` fields above close that gap by exposing the recorded totals — read from `UsageStore.get_conversation_usage(conversation_id)`, never recomputed — through the same channel that already carries per-turn context, relying on the additive `unpriced` column described below.
...
- **`session_cost_usd` is exact for rows written after the `unpriced` column existed, a floor for anything spanning rows written before it.** A turn whose provider reported no cost is recorded as `cost_usd=0.0` — the same repo-wide convention every `record_usage()` caller uses (native `agent_loop.py`, `synthesizer.py`, the CLI-worker executors, and this Hermes path alike; see "Usage and cost reporting" above) — but it is *also* flagged `unpriced=True` at write time, so a later reader does not have to guess which zero-cost rows were actually free. `session_cost_is_lower_bound` reports that flag, summed: `false` means the conversation's recorded usage is a real total, not an estimate. **The gap that remains is retroactive, not ongoing:** a row written before the `unpriced` column existed has no way to be reclassified — it reads as priced/`0` regardless of what actually happened — so any window whose sum includes such a row is still only a floor, even when `session_cost_is_lower_bound` reads `false`, because that flag can only see what was recorded after the column existed. There is no way to distinguish a conversation with such old rows from one that just never hit an unpriced turn; treat any long-lived conversation's `false` with that caveat in mind.
...
| Direct MCP clients (Claude Code, Codex, managed agents) | `mcp_server.py` → task HTTP endpoints | Reads enriched tasks/children and uses explicit project actions, including pause/resume. Project cancellation is preview/confirm and reports durable partial failures. Curated task create/update, and `lifeos_project_resume`, carry a caller-asserted `X-LifeOS-Agent-Session` header whenever this server has a worker identity — never from interactive operator MCP — and the API refuses `#hermes` and an out-of-scope paid route on an agent-attributed project child, and refuses an agent-attributed project resume outright (403). `lifeos_project_pause` does not carry the header: an agent may pause a project. See [API Reference — Agent-session attribution and project-child guards](../product/api-reference.md#agent-session-attribution-and-project-child-guards). |
...
| Turn-context delivery | Folded directly into the system prompt | None | Via the `lifeos_context.turn` envelope |
| Handoff (CLI engine) | Yes | No | No |
| Orchestrating personas | Runs natively (spawns a background session) | N/A — never selectable | Reaches Hermes like any other persona; Hermes drives it via `lifeos_agent_spawn` |
...
| Conversation history LifeOS-owned | Yes | No | Yes (tee-persisted) |
| Survives a client disconnect | Yes, every modality (see above) | No — no observer to persist a detached turn's output | Yes, every modality (same as LifeOS) |
| Explicit cancel (`POST .../cancel`) | Yes | Yes (the endpoint doesn't distinguish backends — but nothing detaches here to cancel) | Yes |
...
`restoreBackendConversation()` renders the stored conversation on a switch **to either `lifeos` or `hermes`** (both are LifeOS-owned history) — only switching to `agent` keeps the fresh-view-but-retain-the-id behavior, because that backend's history isn't persisted here.
...
- [Agent Worker — Technical](agent-worker.md) — The `?conversation=<id>` deep link a board-assigned Hermes card's open action lands on, additive to this doc's SSE contract
...
### Code References
- [Voice proxy](../../../api/routes/voice.py) — `/api/voice/*` reverse proxy and `_VoiceTurnPersister`
...
- [Hermes proxy](../../../api/routes/hermes_proxy.py) — Hermes text-backend routes, `lifeos_context` envelope, turn persistence, and `POST /api/hermes/resolve-persona` / `POST /api/hermes/register-persona-message` (Hermes-Telegram persona selection + reply-thread inheritance)
- [Hermes persona thread store](../../../api/services/hermes_persona_thread_store.py) — the `(chat_id, message_id) -> persona_id` mapping behind reply-thread inheritance
