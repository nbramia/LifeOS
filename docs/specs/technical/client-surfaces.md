...
- `evidence` is the structured result bundle collected for that revision — pull requests, the review outcome, the candidate verifier's lane result, the merged commit, the deployment verifier's `RuntimeEvidence`, and the revert handle. `shipped` requires all of them to agree on the merged commit, including the revisions the processes are actually running; a merge whose deployment failed reports the deployment failure instead.
