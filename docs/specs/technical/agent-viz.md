...
**Click** toggles/opens the panel synchronously — no timer (the old 220ms double-click-disambiguation delay is gone). **Double-click** on a non-subagent node whose `engineOf(d)` is `claude_code` or `codex` calls `focusSessionQuick(d)` (`POST /sessions/{id}/focus`); the panel having already opened from the first click of the pair is expected and unavoidable, not a bug.
