...
`renderBulkBar()` toggles `#board-bulk-bar` and `#board-drop-tray`'s `hidden` attributes as a pair: at most one shows at a time, exactly like the tray already occupies the same flex slot at the bottom of the Board tab that the bulk bar now shares. Both elements carry their own `.board-bulk-bar[hidden]`/`.board-drop-tray[hidden] { display: none; }` CSS rule — each element's own unconditional `display: flex` would otherwise beat the browser's default `[hidden]` behavior, since an author declaration always wins over the user-agent stylesheet regardless of selector specificity or source order.
