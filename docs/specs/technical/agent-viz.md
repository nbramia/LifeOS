...
Response: `{killed: [session_ids], failures: [{session_id, error}], reason: <input>}`. Cancel's teardown call passes reason `"cancelled from the board"` and folds `killed`/`failures` into its own response alongside `id`/`lane`/`status`/`tags` — see [Card action policy](#card-action-policy).
