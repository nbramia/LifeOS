...
- **`assignment.js`** — the card-assignment pickers (engine/model/effort/host) that `board.js` mounts into the drawer; writes `model`/`effort`/`host` (+ `assigned_by`) through `PUT /api/tasks/{id}` and reads `GET /api/agents/models` for the model options and `GET /api/agents/hosts` for the host picker's options (#883). Standalone module from #851, wired in by #859. Saves are serialized through a single in-flight promise chain (#901 round 3, finding A5) rather than fired independently — a queued save reads the controls' CURRENT values only once its turn arrives, then PUTs those, so at most one save is ever outstanding; a rejected save reverts each picker to exactly what the last *successful* save sent, never to a live control value some other save's resolution happened to catch mid-flight.
...
| `model` | str \| null | LifeOS only. Board-assignment model id (#851) from `Session.model` — the operator's model *picker* value, not what actually ran. Does not feed the Hermes badge — see `_model_label_for_routing` below. |
...
- `_model_label_for_routing(routing)` (#863; #863 review dropped its `model` parameter) — `local` → `Local`; `ask` → `Waiting on you` (a session parked waiting on the operator has no model running, so it must never render a Claude-tier guess); `remote` → `settings.remote_llm_label` (falls back to `Remote` if unset or settings import fails); `hermes` → `Hermes`, plain, always; `claude_code`/`code` → `Claude Code`; `codex` → `Codex`; otherwise derives the badge from `settings.agent_managed_model`, falling back to `Claude` if the model name doesn't match any known family. `Session.model` (the board's model *picker* value) is exposed separately as the `model` field above but does not feed this badge — `HermesExecutor` never reads or writes it, and a conversation-rooted Hermes session (the dominant path) passes no `model` at all. (#863 review round 2, finding O) An earlier version of the Hermes branch appended `· <model>` from `api/services/model_readout.py`'s `_last_observed_hermes_chat_model()` — a single process-wide "last observed" value written by *any* Hermes turn (an agent-worker session and `/chat`'s Hermes proxy alike), not a per-session attribution. A finished session's badge could retroactively change to whatever model an unrelated, later Hermes turn reported — `model_readout.py`'s own docstring rejects exactly this kind of cross-surface borrowing as dishonest, so the suffix was removed rather than qualified. The readout still exists and is correctly scoped on the `/models`/`/api/health` surface it was built for; it just doesn't feed this per-node badge.
...
`nodeLabel(d)` (#863) picks the rendered label, first non-empty of: `custom_label` → `short_label` → `label` → `prompt_preview` → `model_label` → `routingLabel(d.routing)` → `session_id.slice(0, 8)`. **Both** `short_label` and `label` are skipped when they're not a real label but the raw id the row fell back to — equal, by trimmed string equality, to `session_id`, to `session_id` with a `cc:`/`cx:` prefix stripped, or to `task_id` (an earlier version of this guard used `session_id.startsWith(label)`, which never matches a prefixed CLI session id and let raw ids leak through as labels; a version after that applied the guard to `label` only, letting a cached raw-id `short_label` leak through one precedence slot higher, since it's tried first — #863 review finding M). On the server side, `agent_viz_summary.py`'s `_fallback_label` independently refuses to hand back a raw identifier as `short_label` in the first place — it returns `""` for a whitespace-free input (a session/task id has no word boundaries of its own) or for an input that's genuinely non-empty but tokenizes to zero words, which includes any non-Latin (CJK/Cyrillic/Greek/Arabic) or emoji-only title, not only a raw id (#863 review round 3 finding Q — an earlier version returned `"Untitled"` for that case, which sits ABOVE `label` in this precedence and clobbered the real title); `"Untitled"` is reserved for a genuinely empty/whitespace-only input, where there's no real title in `label` to fall through to. Never emits a literal `'?'` — the previous version special-cased an unresolved CLI `model_label` (`"Claude Code"`/`"Codex"`) into a bare `'?'` glyph instead of falling through to any of the other sources on the row. In practice the `session_id.slice(0, 8)` tail is unreachable: `routingLabel()` returns `'Claude'` for any unrecognized routing and never an empty string, so it always wins before the final fallback is tried — the fallback remains as a safety net, not something a node actually shows (#863 review).

`web/agents/graph.js`'s search-results dropdown (`sessionDisplayName`) shares `nodeLabel`'s `isRawIdValue` guard (#863 review round 3 finding V) for the dropdown's sort order and for a title whose matched field is itself empty. It does not close every path: `buildSearchResults`'s 'label' search tier matches on `custom_label || label`, and `renderSearchResults` then renders that same truthy `label` value directly rather than through `sessionDisplayName` — so a search that matches on a raw-id `label` still surfaces it verbatim in that one tier. Documented, not fixed (out of scope for #863).
