...
- **`lanes.js`** — the board's `LANES` table and `LANE_COLORS` palette, imported by `board.js` (lane columns), `graph.js` (lane sub-band layout), and `graph_encoding.js` (node fill + legend), so board and graph agree on which lanes exist and what each looks like.
...
A `<div id="graph-panel-actions">` sits inside `#panel-outer`, above the transcript panel proper, and is populated by `renderPanelActions(source)` — called from `openPanel`, `selectAnchor`, and `applySnapshot` (to keep it current across a snapshot tick while a source stays selected) — with a source that's either a session dict or a card anchor object:
...
`getFilters()`/`setFilter(key, value)`/`resetFilters()`/`subscribe(fn)` hold the seven shared keys — `search`, `lanes` (array of lane ids), `assignee`, `host`, `engine`, `tag`, `recency` — as one object, persisted whole under `localStorage['lifeos.agents.filters.v1']`. `setFilter` replaces `filters` wholesale (never mutates in place) so every `subscribe` callback, including the caller's own, sees a fresh reference and reconciles its controls against the passed-in state rather than assuming it's someone else's change. Reads and writes are wrapped in try/catch; a corrupt or missing value falls back to defaults.
...
`focusNode(sessionId)` (`graph.js`) is `selectSearchResult`'s model, generalized for a caller with no search UI: it relaxes whichever LOCAL filters (`relaxFiltersFor` — recency/cwd/route/status) and SHARED filters (`relaxSharedFiltersFor` — lane/assignee/host/engine/tag, via `setFilter`) currently hide the session, expands any collapsed ancestor, then `panToNode` + `openPanel`. An id that doesn't resolve to a known session calls `showToast(..., true)` and returns, leaving the default view. `revealCard(cardId, {openDrawer})` (`board.js`) is the board's counterpart: it only relaxes the shared `lanes` selection (adding the card's own lane back if the filter currently hides it — the acceptance criterion is that the card is actually visible, not merely that some filter changed), `scrollIntoView({block: 'nearest'})`s it, applies a `.reveal-highlight` CSS class for ~2s, and opens the drawer when `openDrawer` is true; an unknown card id toasts and returns.
