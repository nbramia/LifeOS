...
| `model` | str \| null | LifeOS only. Board-assignment model id (#851) from `Session.model` — the operator's model *picker* value, not what actually ran. (#863 review) `Hermes · <model>` above no longer sources from this field — see `_model_label_for_routing` below. |
...
- `_model_label_for_routing(routing)` (#863; #863 review dropped its `model` parameter) — `local` → `Local`; `ask` → `Waiting on you` (a session parked waiting on the operator has no model running, so it must never render a Claude-tier guess); `remote` → `settings.remote_llm_label` (falls back to `Remote` if unset or settings import fails); `hermes` → `Hermes` alone, or `Hermes · <model>` when `api/services/model_readout.py`'s `_last_observed_hermes_chat_model()` has one — the model actually reported on the most recent real Hermes chat turn *in this process* (parsed from a `usage` event by `hermes_proxy.py`'s `_HermesTurnPersister`; a process-wide "last observed" value, not a per-session attribution, and read defensively so a lookup failure degrades to plain `Hermes`); `claude_code`/`code` → `Claude Code`; `codex` → `Codex`; otherwise derives the badge from `settings.agent_managed_model`, falling back to `Claude` if the model name doesn't match any known family. `Session.model` (the board's model *picker* value) is exposed separately as the `model` field above but no longer feeds this badge — `HermesExecutor` never reads or writes it, and a conversation-rooted Hermes session (the dominant path) passes no `model` at all, so the old branch was unreachable there and, when it did fire, asserted a model Hermes was never actually told to use.
...
`nodeLabel(d)` (#863) picks the rendered label, first non-empty of: `custom_label` → `short_label` → `label` (skipped when it's not a real label but the raw id the row fell back to — `label` equal, by trimmed string equality, to `session_id`, to `session_id` with a `cc:`/`cx:` prefix stripped, or to `task_id`; an earlier version of this guard used `session_id.startsWith(label)`, which never matches a prefixed CLI session id and let raw ids leak through as labels, #863 review) → `prompt_preview` → `model_label` → `routingLabel(d.routing)` → `session_id.slice(0, 8)`. Never emits a literal `'?'` — the previous version special-cased an unresolved CLI `model_label` (`"Claude Code"`/`"Codex"`) into a bare `'?'` glyph instead of falling through to any of the other sources on the row. In practice the `session_id.slice(0, 8)` tail is unreachable: `routingLabel()` returns `'Claude'` for any unrecognized routing and never an empty string, so it always wins before the final fallback is tried — the fallback remains as a safety net, not something a node actually shows (#863 review).
