...
A snooze is a wake-up time stored on the task as the custom field `[snoozed_until:: <ISO-8601 with offset>]` (`agent_board.SNOOZED_UNTIL_FIELD`) — status and tags are untouched. `agent_board.parse_snoozed_until` parses it, rejecting a missing, unparseable, or offset-less value; `agent_board.is_snoozed(fields, now=None)` is true only while the parsed value is strictly after `now` (real clock by default, injectable for tests) — a past value is treated exactly as absent, and is never cleaned up.

`derive_lane(status, tags, fields=None, now=None)` checks `is_snoozed` first, ahead of every other rule (Review, Human queue, In progress, Done, Assigned, Unassigned) — a future wake-up time wins regardless of status/tags. This is safe in practice because every write path that would otherwise move a card into In progress or Done through the board's own guarded surface also clears the field: `plan_lane_move` clears it on every successful move (and refuses a direct move into `snoozed` the same way it refuses `review`/`scheduled`), `accept_board_card`'s Done write clears it, and `cancel_board_card`'s write clears it — a cancelled or accepted card never carries a lingering future value. `derive_lane` itself does not special-case status — an untouched stale value would still win, by design, so a caller that bypasses these guarded paths cannot get a silently-wrong answer out of derivation itself.

`PUT /board/cards/{id}/snooze` (body `{until}`) validates `until` (parseable, offset, strictly future — all 4xx, no write, on failure) before reading the card, then refuses (409) if `derive_lane(task.status, task.tags)` — evaluated with `fields=None`, i.e. ignoring any snooze already in effect, so re-snoozing an already-snoozed eligible card still works — is `in_progress` or `done`; a `card_id` that never resolves through `TaskManager` (a scheduler entry) 404s the same as any other missing card. The eligibility check is re-run inside `TaskManager.update`'s `_precondition` against the exact snapshot the write lands on, so a card the worker claims into `in_progress` in the same window loses the race. On success the write is `fields={"snoozed_until": <until>}` alone — no `status`/`tags`/`notes` kwarg — so those fields, and the file's notes body, are byte-for-byte unchanged apart from the new inline field and the routine `updated_at` bump. `DELETE /board/cards/{id}/snooze` clears the field (`fields={"snoozed_until": None}`) and is a genuine no-op (no `TaskManager.update` call, `updated_at` unchanged) when the field is already absent.
