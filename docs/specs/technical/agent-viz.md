...
| `POST /board/cards/{id}/cancel` | #881, round-2 hardened. Reads the task; ownership is checked BEFORE the idempotence short-circuit — `is_review_pending`/`is_agent_owned` on the raw tags always run first, so a `me` card or a Review card that somehow already carries `status="cancelled"` gets its real 409 instead of a misleading idempotent 200 (round-1 finding RC2). Only once that passes does `status == "cancelled"` short-circuit to a no-op (mirrors accept's idempotence, touches no session). Otherwise calls `evaluate_card_action(..., "cancel")`, which also refuses an already-**finished** card — `status` `"done"` (e.g. accepted) as well as `"cancelled"` — with `CANCEL_ALREADY_FINISHED_ERROR` (round-1 finding RC2; narrows the issue's literal "any agent-assigned card not in Review" wording — flagged for the operator). Finds the card's live LifeOS-agent session with a direct `SessionStore.get(card_id)` primary-key read — `task_id` is the `sessions` table's PK — not the 200-row, most-recently-started `_build_snapshot()` window `_task_card` uses for display, which could silently miss an older still-running session (round-1 finding AR1, previously via a now-removed `_find_latest_session_for_task` helper). If one exists and isn't terminal, tears down its whole subtree via `_kill_session_subtree` (shared with the kill endpoint, reason `"cancelled from the board"`). Separately queries `SessionStore.list_cli_sessions_for_task(card_id)` for any non-`ended` row — a live cc:/cx: CLI session (opened via the drawer's Open button) lives in the separate `cli_sessions` table, keyed by its own session_id, not task_id, so the PK lookup above can never find it, and Cancel can't kill a CLI process (a separate, deferred issue) — each one found is appended to `failures` with a reason naming it, rather than the endpoint silently claiming a teardown it didn't perform (round-1 finding AR5). Strips `agent-running`/`agent-blocked`/`human` from the tags (any of which would otherwise outrank Done in `derive_lane`) and writes `status="cancelled"` — `TaskManager` stamps `[cancelled:: <date>]` and the `- [-]` checkbox. Response: `{id, lane, status, tags, killed, failures}`. |
...
`agent_board.evaluate_card_action(current_status, current_tags, action, target_lane=None)` is the ONE decision every server write path that can touch an agent-owned card's lane, assignee, or status consults — `None` means allowed, `(http_status, detail)` means refused with that exact status/detail and no write. `action` must be one of `CARD_ACTIONS` (`"lane_move"` [needs `target_lane`], `"assignee_change"`, `"field_edit"`, `"cancel"`) — any other value raises `ValueError` rather than falling through to an implicit allow (round-1 finding RC1: the pre-round-2 version defined `CARD_ACTIONS` but never validated against it). Its rule precedence, evaluated top to bottom:

1. An unknown or undroppable (`review`/`scheduled`) `target_lane` — 400 (lane_move only, unchanged from pre-#881).
2. `claimed` — every `lane_move` target is refused, and so is `assignee_change`/`field_edit`; `cancel` is still allowed (it's how a human gets rid of a claimed card without dragging it anywhere). `is_claimed(status, tags)` is true for `agent-running`/`agent-blocked` tag present (unchanged), OR (round-1 finding AR4) `status == "in_progress"` on an agent-owned, non-review card — `cli_session_event` sets that status the first time the drawer's Open button spawns a CLI session on a `#claude`/`#codex` card, before the worker itself ever adds `agent-running`; without this, an opened-but-not-yet-claimed CLI session stayed fully editable, recreating exactly the desync #881 exists to prevent. The non-review exclusion matters because the same status can linger at `in_progress` after the worker completes a card that was earlier opened via CLI (nothing resets it) — without it, a Review card could be swallowed by this check and lose the accept-by-drag Done carve-out (see `tests/test_agent_board.py::test_cli_opened_review_card_still_treated_as_review_not_claimed`).
...
`lanes` lists ONLY refused lanes (round-1 finding RC5) — `unassigned`/`assigned` are allowed for this card, so they're simply absent rather than present-and-`true`; a lane an every-state 400 (`review`) still appears whenever it's evaluated, which is unconditionally. This cut the 555-1261 bytes per card `GET /board/stream` re-serializes to compute its change signature every 0.5s, and reflects what the client already assumed (`onCardDropped` treats a missing lane entry as allowed). Because Scheduled isn't in `TASK_LANES` at all, it NEVER appears in `lanes` regardless of state — `web/agents/board.js`'s `DIRECT_LANE_IDS` set (also used to gate the composer's lane picker and per-lane **+** buttons) is what actually keeps a drag onto Scheduled from round-tripping to the server just to 400 (round-1 finding AR6).

`web/agents/board.js` and `web/agents/assignment.js` read `card.policy` directly — a drag checks `card.policy.lanes[targetLane]?.allowed` before issuing the PUT (a fast client-side path; the server still refuses independently, so a stale board is caught by the normal error-toast path; Review/Scheduled are refused via `DIRECT_LANE_IDS` before this check even runs), the drawer's assignee select, the Tags field, and the assignment pickers (including the engine select, round-1 finding RC8) disable themselves and render `reason` as visible text — in its own neutral `.drawer-field-reason` element, never the `.assignment-error` element reserved for an actual failed save (round-1 finding RC6) — when their policy entry is refused, the Cancel button is offered exactly when `policy.cancel.allowed`, and Resolve (a drop onto Done under the hood) is gated on `(card.policy.lanes.done ?? {}).allowed !== false` — guarding the now-possibly-absent `.done` leaf (round-1 finding RC7, a direct consequence of RC5's shape change). Clicking Cancel re-renders the open drawer explicitly afterward rather than relying on `fetchBoard()`'s own `updateOpenDrawer`, which skips the rebuild while the just-clicked button still holds focus — without it a stale Open button on a card that just moved to Done would 409 on the next click (round-1 finding AR7). Schedule cards carry no `policy` at all (there's nothing to derive it from), and any task card lacking one (e.g. a pre-#881 fixture) is treated as fully allowed everywhere — the frontend never re-implements the rules, so an absent policy defaults open rather than guessing.

The drawer's Tags field (`web/agents/board.js`) is gated by `policy.assignee` — the same policy entry the Assignee select uses, since a tags-only edit can change the same claim state. It never renders a worker lifecycle tag (`agent-running`, `agent-blocked`, `agent-completed`, `accepted`, or any other tag matching `/^(agent(-[\w-]+)?|accepted)$/i`) as an editable token — mirroring how it already excludes assignee-name tokens — rejects one if typed (round-1 finding AR3, hostile-reviewer reproduction: typing `agent-running` into a `me` card's Tags field used to grant it a claim tag the worker never gave it), and always re-appends the card's own lifecycle tags from `card.tags` on save regardless of what's in the box, so an edit on a still-editable card (e.g. Review, which isn't claimed) can't silently strip one either.

### `assigned_by: "board"` guard on `PUT /api/tasks/{id}`

The board's assignment pickers (`web/agents/assignment.js`) write `model`/`effort`/`host`, and its engine picker writes the assignee tag, through `PUT /api/tasks/{id}` rather than the lane endpoint — every one of those writes stamps `fields.assigned_by: "board"`. `api/routes/tasks.py`'s `update_task` checks for that marker (case-insensitive, stripped) ONLY when `request.fields` is present, so a request without it — an agent's own writes, a vault-triggered reindex, any pre-#881 caller — never pays for the extra `TaskManager.get` this guard needs and behaves exactly as before. When the marker is present: a patch touching `model`, `effort`, or `host` runs `evaluate_card_action(..., "field_edit")`; a patch whose `tags` would change the normalized assignee-tag SET or claim-tag SET (`agent_board.normalize_tags(new) & {ASSIGNEE_TAGS...}` vs. the old, and likewise for `{RUNNING_TAG, BLOCKED_TAG}`) runs `evaluate_card_action(..., "assignee_change")`. Either refusal raises the `HTTPException` before `TaskManager.update` is ever called — no partial write. The guard used to compare only `derive_assignee(new) != derive_assignee(old)` (a single, first-match-wins value); round-1 findings AR2 and AR3 showed that's blind to a SECOND assignee tag added alongside the first (`derive_assignee` still resolves to the original) and to a claim tag (`agent-running`/`agent-blocked`) dropped from the tags list while the derived assignee is unchanged — both now trip the guard because the tag SETS differ even when the single derived value doesn't.
...
- `_model_label_for_routing(routing, hermes_model=None)` — `local` → `Local`; `ask` → `Waiting on you` (a session parked waiting on the operator has no model running, so it must never render a Claude-tier guess); `remote` → `settings.remote_llm_label` (falls back to `Remote` if unset or settings import fails); `hermes` → `Hermes · <hermes_model>` if `hermes_model` is truthy, else plain `Hermes`; `claude_code`/`code` → `Claude Code`; `codex` → `Codex`; otherwise derives the badge from `settings.agent_managed_model`, falling back to `Claude` if the model name doesn't match any known family. `Session.model` (the board's model *picker* value) is exposed separately as the `model` field above but does not feed this badge — `HermesExecutor` never reads or writes it, and a conversation-rooted Hermes session (the dominant path) passes no `model` at all.
  ...
  - `model_label` reaching `Hermes · <model>` in the API response is not the same as it rendering anywhere: `web/agents/graph.js`'s `nodeLabel()` consumes it only as the fifth-priority NAME fallback (behind `custom_label`, `short_label`, `label`, `prompt_preview`), and the side panel's own Routing badge is computed client-side from the plain routing value and never reads `model_label` at all. A board-assigned Hermes session's `label` is its card title, so in practice it never reaches that fallback (only a row whose card has left the task index — and so has no usable `label` or `prompt_preview` — falls through to it); nothing else on the page shows the enriched value — see [agent-viz.md](../product/agent-viz.md#graph-tab--side-panel) for the product-facing description.
