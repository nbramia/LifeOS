...
2. **Transcribe-so-far.** The candidate's audio is POSTed to the same bare-STT route Listening's wake check uses, `POST /api/voice/transcribe` (see below) — no conversation/turn artifacts either way.
...
### The bare-STT gateway route

Both Listening (above) and smart turn endpointing depend on the same **transcribe-only** gateway route, `POST /api/voice/transcribe` — no LLM call, no TTS, no conversation/turn persistence, safe to call every second or two without creating any artifacts. whisper-relay's `POST /api/voice/turn` and `/turn/stream` always run the full STT→LLM→TTS pipeline instead; `/api/voice/transcribe` is the same audio normalization + `STTAdapter.transcribe()` step `turns.py` uses internally, minus everything after it — multipart `audio` file in, `{"transcript": "..."}` out. LifeOS's `/api/voice/*` proxy forwards it generically (`api/routes/voice.py`), with no route-specific logic on the LifeOS side. If the gateway is unreachable or its STT engine fails, the call 404s or 503s; `transcribeClip()` in `web/chat/voice.js` treats either as "no transcript" and moves on rather than erroring, so Listening simply doesn't trigger and an endpointing candidate check doesn't get a "complete" verdict from step 3 above for that round. The hard cap in step 4, though, is a pure client-side silence timer with no dependency on this route — it still fires on schedule regardless, so under Auto a recording that goes quiet for `LIFEOS_VOICE_ENDPOINT_HARD_CAP_MS` still ends and sends itself; only the *early*, mid-sentence completeness signal depends on a reachable transcribe call.
...
A spoken turn on the Hermes backend calls the Hermes harness directly rather than routing through LifeOS's **own** Hermes proxy (`POST /api/hermes/ask/stream`) the way a typed Hermes turn does — that proxy is the seam where persona resolution and the `lifeos_context` envelope live (see [client-surfaces.md](../specs/technical/client-surfaces.md)) — so a spoken Hermes turn carries no persona context or spoken-style rules. Conversation persistence doesn't depend on that seam, though — `api/routes/voice.py` tees `POST /api/voice/turn/stream` into the conversation store directly, so a Hermes-backend voice conversation still survives a page refresh even without persona context.
...
| `Listening` never triggers recording | whisper-relay doesn't serve `POST /api/voice/transcribe` (too old, or unreachable) — see "The bare-STT gateway route" above; upgrade or restart the gateway. |
| Auto-mode recording never ends itself on a finished sentence, only on the hard cap or a tap | Same cause as above — the hard cap alone still fires on schedule regardless. |
