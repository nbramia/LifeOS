...
| `ANTHROPIC_API_KEY` | str | — | Required when `LIFEOS_LLM_BACKEND=anthropic`. Also the preferred client for specialized calls (relationship insights, fact extraction, tone analysis — [ADR-023](../adr/023-specialist-call-fallback.md)) regardless of `LIFEOS_LLM_BACKEND`; when unset, those calls fall back to the local llama-server if reachable, else the remote provider below, instead of silently producing nothing (#772). Web search has no local equivalent and is unaffected — see below. |
