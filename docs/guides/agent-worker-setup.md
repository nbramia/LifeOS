...
# LIFEOS_AGENT_DEFAULT_BUDGET_DOLLARS=10.00
...
Setting `TYPESAFE_API_KEY` in `.env` enables typed judgments via TypeSafe's Jev (https://docs.typesafe.ai) for surfaces that opt into it. Each surface that can use Jev is independently opt-in on top of the key, and nothing is sent to TypeSafe unless the key is set — an install without it behaves exactly as one without any Jev-backed judgment enabled. Preflight's destructiveness judgment is one such surface: it runs in `shadow` mode by default once a key is set (`LIFEOS_AGENT_JEV_DESTRUCTIVE_GATE`), logging alongside the regex sanity gate rather than acting on its own — see [agent-worker.md § Preflight](../specs/technical/agent-worker.md#preflight). The chat orchestrator has its own shadow surface, off by default: `LIFEOS_JEV_ORCHESTRATOR=shadow` records pre-turn and in-loop Jev judgments as perf-trace spans without changing which tools, rounds, or model a turn gets — see [observability.md § Jev Orchestrator Shadow](../specs/technical/observability.md#jev-orchestrator-shadow).

**Task routing.** With the key set, one Jev call per task (`jev_task_routing.judge_task`, memoized per task title) answers where the task belongs, how difficult it is, and which preset class fits. `directory_resolver.resolve_working_directory` and `agent_worker/claude_code_spawn.should_use_plan_mode` consume the location and difficulty answers respectively (0.6 confidence floor); `preflight._apply_preset_class` consumes the preset-class answer, gated at a higher 0.7 floor plus a software-work guard (a task the judgment itself flags as likely software work never gets a narrowed tool set — see [agent-worker.md § Preflight](../specs/technical/agent-worker.md#preflight)). Each falls back to its existing keyword/tag logic below its own floor, or with no key at all.

The working-directory location choice is built from the union of the operator's GitHub repositories and the locally scanned project directories. Set `LIFEOS_GITHUB_OWNER` to the GitHub login whose repos to list; left empty, it's resolved from `gh api user -q .login` instead (both the login and the repo listing are cached on disk for 24 hours, written atomically). A repository Jev names that isn't cloned on this host still resolves to a path under `LIFEOS_CODE_DIR` — the worker clones it there (`gh repo clone`) right before spawning a CLI-routed (`#claude`/`#codex`) session; a clone failure parks the task at `#agent-blocked` naming the repository, rather than spawning into a directory that doesn't exist. **Remote-host limitation:** this clone-on-demand only runs when the executing host is the worker process itself. A session dispatched to a host registered in `LIFEOS_AGENT_HOSTS` runs the CLI over ssh on that other machine, whose filesystem this process can't reach — an uncloned GitHub-only repository is never named for that spawn in the first place (the resolution falls back to the keyword cascade instead), so such a session either already has the repository cloned there or ends up somewhere the keyword guess picks.
...
server's own `env_vars` config key; for a worker-launched `#codex` session,
`CodexExecutor` supplies that key itself via a `-c
mcp_servers.lifeos.env_vars=[...]` override on the codex argv, so this
