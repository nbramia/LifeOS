...
- **Approval is what opens the gate.** Until a revision is approved, LifeOS refuses to spawn a worker to implement that repair; read-only investigation continues. This is a gate on what LifeOS dispatches, not a sandbox over the shell a running session already has — merging, deploying, and restarting are work the supervisor does inside its own session.
