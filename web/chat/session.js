// Shared chat state for the /chat surface.
//
...
// sessionStorage; backend × conversation persistence is a separate concern.
...
export const state = {
  ...
  isLoading: false,
  // When set, the main chat view is showing an agent thread: the
  ...
  // -- an external backend that genuinely can't price a turn sends no
...
// Per-query selectors. `personaId` is the selected persona; `backend`
// is reserved for the Voice|Text follow-on. `personas` caches the list
...
export const config = {
  ...
  backend: null,
  personas: [],
  // Voice|Text input mode; restored from sessionStorage by initVoice().
  voiceMode: false,
