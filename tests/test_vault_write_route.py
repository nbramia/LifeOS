...
def app_and_vault(monkeypatch):
    ...
    Monkeypatch the shared `settings` singleton's .vault_path attribute
...
# ---------------------------------------------------------------------------
# Personal/Journal/ is reserved — those files are generated by
...
#
# The reservation only applies when this install actually
...
# server both are plausibly already configured, so these tests must pin
# both signals explicitly rather than trust the ambient state — otherwise
# they could give a false pass in this worktree while actually failing on
# that server.
# ---------------------------------------------------------------------------
...
def _enable_journal_persona(monkeypatch):
    ...
    **os.environ}`, and monkeypatch.setenv always wins that merge.
    ...
    the Telegram-bot signal alone is sufficient —
...
def test_rejects_writes_into_reserved_journal_dir(app_and_vault, bad_path, monkeypatch):
    """Regression guard: with the journal persona enabled, the write must
    be rejected with this exact message text, not just a substring match —
    a substring check wouldn't catch the message itself regressing."""
...
def test_allows_writes_into_journal_dir_when_persona_not_enabled(app_and_vault, tmp_path, monkeypatch):
    """When neither pipeline that generates Personal/Journal/ files is
...
class TestJournalEnabledSignal:
    ...
    independent signals — the route-level tests above
