"""Tests for the per-session local-executor thinking override:
...
(non-LocalLLMClient) test double, so every other local_executor test
...
def test_remote_forced_route_never_receives_enable_thinking(tmp_path, monkeypatch):
    """The remote-forced route is also a LocalLLMClient instance but
...
def test_non_local_llm_client_fake_never_receives_enable_thinking(tmp_path):
    ...
    must not be called with it — this is what keeps every other
