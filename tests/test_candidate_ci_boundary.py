...
@pytest.mark.unit
def test_candidate_execution_fans_out_into_a_concurrent_browser_free_and_four_shard_matrix():
    """The execution stage is one job body (so every leg automatically
    repeats the same checkout/provenance/install/prove steps and the same
    `contents: read`-only permissions -- there is no second copy of any of
    that to drift out of sync) instantiated as several concurrent legs: the
    whole server-free browser lane, and fast-unit split into four disjoint
    shards."""
    import yaml

    workflow = yaml.safe_load((ROOT / ".github/workflows/candidate-verification.yml").read_text())
    job = workflow["jobs"]["execute-candidate"]
    matrix = job["strategy"]["matrix"]["include"]
    assert job["strategy"]["fail-fast"] is False

    browser_free = [leg for leg in matrix if leg["lane"] == "browser-free"]
    assert len(browser_free) == 1
    assert browser_free[0]["shard_count"] == 1
    assert browser_free[0]["shard_index"] == 0

    fast_unit = [leg for leg in matrix if leg["lane"] == "fast-unit"]
    assert len(fast_unit) == 4
    assert all(leg["shard_count"] == 4 for leg in fast_unit)
    assert sorted(leg["shard_index"] for leg in fast_unit) == [0, 1, 2, 3]

    assert set(leg["lane"] for leg in matrix) == {"browser-free", "fast-unit"}
    assert len({leg["label"] for leg in matrix}) == len(matrix), "matrix leg labels must be unique"


@pytest.mark.unit
def test_candidate_execution_timeout_is_short_enough_for_a_fast_failure():
    """A per-job ceiling well under the old single-job 65 minutes, so a
    genuinely stuck shard fails fast and legibly instead of being cancelled
    near a ceiling barely above the observed worst case."""
    import yaml

    workflow = yaml.safe_load((ROOT / ".github/workflows/candidate-verification.yml").read_text())
    assert workflow["jobs"]["execute-candidate"]["timeout-minutes"] <= 20


@pytest.mark.unit
def test_candidate_execution_measures_real_width_before_verifying():
    """Every leg records nproc and available memory before running tests,
    and the worker count it derives from that measurement -- evidence the
    choice is real rather than a hardcoded guess."""
    import yaml

    workflow = yaml.safe_load((ROOT / ".github/workflows/candidate-verification.yml").read_text())
    steps = workflow["jobs"]["execute-candidate"]["steps"]
    names = [step.get("name") for step in steps]
    measure_index = names.index("Measure the runner's real width")
    verify_index = names.index("Verify this leg's assigned lane shard")
    assert measure_index < verify_index

    measure_script = steps[measure_index]["run"]
    assert "nproc" in measure_script
    assert "free -h" in measure_script
    assert 'echo "WORKERS=$WORKERS" >> "$GITHUB_ENV"' in measure_script

    verify_script = steps[verify_index]["run"]
    assert '--workers "$WORKERS"' in verify_script
    assert "workers=$WORKERS" in verify_script


@pytest.mark.unit
def test_candidate_execution_shards_every_matrix_leg_and_opts_browser_free_into_parallel():
    """Every leg passes its own exact shard to the verifier, and only the
    browser-free leg opts into the parallel-worker mode -- fast-unit shards
    already get their concurrency from being separate jobs, not from this
    flag, which only changes browser-free's own in-job worker behavior."""
    import yaml

    workflow = yaml.safe_load((ROOT / ".github/workflows/candidate-verification.yml").read_text())
    steps = workflow["jobs"]["execute-candidate"]["steps"]
    verify_step = next(s for s in steps if s.get("name") == "Verify this leg's assigned lane shard")
    script = verify_step["run"]
    assert "--shard-index \"$SHARD_INDEX\"" in script
    assert "--shard-count \"$SHARD_COUNT\"" in script
    assert "--lanes \"$LANE\"" in script
    assert "--parallel-browser-free" in script
    assert 'if [ "$LANE" = browser-free ]; then' in script


@pytest.mark.unit
@pytest.mark.parametrize("lane, expect_parallel_flag", [("browser-free", True), ("fast-unit", False)])
def test_verify_step_script_opts_browser_free_into_parallel_and_leaves_fast_unit_alone(lane, expect_parallel_flag):
    """Execute the ACTUAL "Verify this leg's assigned lane shard" step
    script -- extracted from the real workflow YAML, not hand-copied --
    against a fake verifier binary that only records its argv, proving the
    real conditional actually gates `--parallel-browser-free` on the lane
    rather than merely mentioning the flag somewhere in the script text."""
    import yaml

    workflow = yaml.safe_load((ROOT / ".github/workflows/candidate-verification.yml").read_text())
    steps = workflow["jobs"]["execute-candidate"]["steps"]
    script = next(s for s in steps if s.get("name") == "Verify this leg's assigned lane shard")["run"]

    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        fake_repo = tmp_path / "trusted-runner" / "scripts"
        fake_repo.mkdir(parents=True)
        argv_capture = tmp_path / "argv.txt"
        (fake_repo / "verify_candidate.py").write_text(
            "import sys\n"
            f"open({str(argv_capture)!r}, 'w').write(' '.join(sys.argv[1:]))\n"
        )
        env = {
            **os.environ,
            ...
            "CANDIDATE_SHA": "c" * 40,
            "TRUSTED_RUNNER_SHA": "b" * 40,
            "WORKERS": "3",
            "LANE": lane,
            "SHARD_INDEX": "1",
            "SHARD_COUNT": "4",
        }
        result = subprocess.run(["bash", "-eo", "pipefail", "-c", script], cwd=str(tmp_path), env=env, capture_output=True, text=True)
        assert result.returncode == 0, result.stdout + result.stderr
        argv = argv_capture.read_text()
        assert f"--lanes {lane}" in argv
        assert "--shard-index 1" in argv
        assert "--shard-count 4" in argv
        assert "--workers 3" in argv
        assert ("--parallel-browser-free" in argv) == expect_parallel_flag
